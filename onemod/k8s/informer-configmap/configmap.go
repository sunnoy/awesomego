...
func (c *Controller) syncToStdout(key string) error {
	...
	if !exists {
		// Below we will warm up our cache with a configmap, so that we will see a delete for one configmap
...
func main() {
	...
	config, err := clientcmd.BuildConfigFromFlags("", "/Users/lirui/Documents/git_repo/xylink/运维日常/kubeconfig/yaml/ops-dev.yaml")
	if err != nil {
		klog.Fatal(err)
	}
	...
	// Let's suppose that we knew about a configmap "mypod" on our last run, therefore add it to the cache.
	// If this configmap is not there anymore, the controller will be notified about the removal after the
